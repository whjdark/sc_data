...
if __name__ == '__main__':
    ...
    model = load_model("forevergod/ak_cnn.h5")
    model.summary()

    if(input("train?") == "no"):
        exit()

    opt = tensorflow.keras.optimizers.Nadam(0.0007)
    model.compile(loss='mean_squared_error',
                      optimizer=opt, metrics=['mae', 'mse'])
    model.fit(x_train, y_train, 64, epochs=100, shuffle=True, callbacks=None)
    
    loss, mae, mse = model.evaluate(x_test, y_test)

    print(loss, mae, mse)

    model.save("ak_cnn.h5")